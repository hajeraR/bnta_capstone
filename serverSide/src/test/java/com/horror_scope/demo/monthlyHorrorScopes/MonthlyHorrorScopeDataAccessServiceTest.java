...
@SpringBootTest
class MonthlyHorrorScopeDataAccessServiceTest {

    @Autowired private JdbcTemplate jdbcTemplate;
    private MonthlyHorrorScopeDataAccessService underTest;

    @BeforeEach
    void setup(){
        underTest = new MonthlyHorrorScopeDataAccessService(jdbcTemplate);
    ...
    @Test
    void selectMonthlyHorrorScopes() {
        List<MonthlyHorrorScope> actual = underTest.selectMonthlyHorrorScope();
    ...
    @Test
    void selectMonthlyHorrorScopeByZodiacSign() {

        List<MonthlyHorrorScope> actual = underTest.selectMonthlyHorrorScopeByZodiacSign("Virgo");
        //Optional<HorrorScope> expected = Optional.of(new MonthlyHorrorScope("Virgo", "./virgoIcon", "./virgoIconDark", "./virgoImage", "./virgoImageDark", "./virgoBackground", "Virgos are a big fan of the french Horn.", "Taurus - after all they are often small and easy to bludgeon.", "Scorpio - You're neighbours but they glare when you pass them in the street.", "The second Tuesday of the month.", "Unlikely.", "You still get phone calls from your mother every Sunday. You haven't had the heart to remind her she's been dead for seven years now.", "Throw a gold coin in the fountain. You must appease the spirit.", "You will have a long running career as a theater musician. Beware of falling light fixtures."));
